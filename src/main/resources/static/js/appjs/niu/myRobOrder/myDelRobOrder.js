			...
			.bootstrapTable(
					{
						...
						url : prefix + "/listDelOrder", // 服务器数据的加载地址
					//	showRefresh : true,
					//	showToggle : true,
					//	showColumns : true,
						iconSize : 'outline',
						...
						columns : [
								...
								},								
								{
									...
									align : 'center',
									formatter : function(value, row, index) {
										var h='<a class="btn btn-danger btn-sm  href="#" mce_href="#" title="恢复" aria-hidden="true" onclick="delOrder(\''+row.orderNo+'\',\''+row.id+'\')"><i class="#">恢复</i></a>';
										return h;
									}
								} ]
					});
...
function delOrder(orderNo,id){
	layer.confirm('确定恢复订单号为:'+orderNo+'？', {
		btn : [ '确定', '取消' ]
	}, function() {
		$.ajax({
			url : prefix+"/delOrder",
